...
def plot_electicity_production_from_oil_sources(df_years):
    country_list = ['United States', 'India', 'China', 'Japan', 'Yemen, Rep.']
    ...
    plt.title('Electricity production from oil sources (% of total)')
    plt.legend()
    plt.show()


def plot_electicity_production_from_hydroeletric_sources(df_years):
    country_list = ['United States', 'India', 'China', 'Japan', 'Yemen, Rep.']
...
def plot_electricity_production_from_oil_sources_and_electricity_production_from_hydroeletric_sources(df_years):
    country_list = ['United States', 'India', 'China']
    oil_sources_indicator = 'Electricity production from oil sources (% of total)'
    hydroeletric_sources_indicator = 'Electricity production from hydroelectric sources (% of total)'
    ...
    for i, year in enumerate(years):
        oil_sources_values = []
        hydroelectric_sources_values = []
        for country in country_list:
            oil_sources_values.append(
                df_years.loc[(country, oil_sources_indicator), year])
            hydroelectric_sources_values.append(
                df_years.loc[(country, hydroeletric_sources_indicator), year])
        rects1 = ax.bar(x - width/2 + i*width/len(years), oil_sources_values,
                        width/len(years), label=str(year)+" "+oil_sources_indicator)
        rects2 = ax.bar(x + width/2 + i*width/len(years), hydroelectric_sources_values,
                        width/len(years), label=str(year)+" "+hydroeletric_sources_indicator)

    ax.set_xlabel('Country')
    ax.set_ylabel('Value')
    ax.set_title(
        'Electricity production from oil sources vs hydroeletric sources')
    ax.set_xticks(x)
...
if __name__ == '__main__':
    ...
    plot_electicity_production_from_oil_sources(df_years)

    # Call plot_electicity_production_from_hydroeletric_sources to create the  plot
    plot_electicity_production_from_hydroeletric_sources(df_years)

    # Call plot_energy_use_and_co2_emissions to create the energy use and CO2 emissions plot
    plot_electricity_production_from_oil_sources_and_electricity_production_from_hydroeletric_sources(
        df_years)

   # select the indicators of interest
indicators = ['Electricity production from oil sources (% of total)',
              'Electricity production from hydroelectric sources (% of total)']

# select a few countries for analysis
countries = ['United States', 'China', 'India', 'Japan', 'Yemen, Rep.']
...
# Use the mean method to find the mean Electricity production from oil sources (% of total) for each country
electicity_production_from_oil_sources = df_years.loc[(
    slice(None), 'Electricity production from oil sources (% of total)'), :]
electicity_production_from_oil_sources_mean = electicity_production_from_oil_sources.mean()
print("\nMean electicity production from oil sources for each country")
print(electicity_production_from_oil_sources)
